...
def test_create_event(client, token_headers, test_user):
    ...
    pipeline_response = client.post("/api/v1/pipelines/", json=pipeline_data, headers=token_headers)
...
def test_get_events_by_pipeline(client, token_headers, test_user):
    ...
    response = client.get(f"/api/v1/events/?pipeline_id={pipeline_id}", headers=token_headers)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert isinstance(data, list)
    assert len(data) >= 1
...
def test_get_event_by_id(client, token_headers, test_user):
    ...
    event_response = client.post("/api/v1/events/", json=event_data, headers=token_headers)
...
def test_update_event(client, token_headers, test_user):
    ...
    response = client.put(f"/api/v1/events/{event_id}", json=update_data, headers=token_headers)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
def test_delete_event(client, token_headers, test_user):
    ...
    response = client.delete(f"/api/v1/events/{event_id}", headers=token_headers)
    assert response.status_code == status.HTTP_200_OK
    
    get_response = client.get(f"/api/v1/events/{event_id}", headers=token_headers)
